...
function recurse(clone, from, to, credentials, internalize) {
  ...
  function end() {
    ...
    deps.forEach(function(dep) {
      if(!internalize && dep.version.match(/[\\\/]/)) {
        return --pending
      }
    ...
    function ondone(dep, err, version) {
      ...
      if(version && data.dependencies[dep.name]) {
        data.dependencies[dep.name] = version
      } else if(version && data.optionalDependencies[name]) {
        data.optionalDependencies[dep.name] = version
      }
