...
function clone(pkg, src, dst, options, ready) {
  ...
  var oldSha = crypto.createHash('sha1')
  var transformedTarball = tar.pack()
  ...
  function runXform() {
    ...
    if(options.transform) {
      options.transform(metadata, updatePackage)
    } else {
      updatePackage(null, metadata)
    }
  }
  ...
  function updatePackage(err, data) {
    ...
    if(pkg.version.match(/[\\\/]/)) {
      newVersion = metadata.version = '0.0.0-' + oldSha.read().toString('hex')
    }
